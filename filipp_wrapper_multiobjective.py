...
class OT2Env(gym.Env):
    ...
    def _calculate_reward(self, current_pos, distance_to_goal, velocity):
        # Primary: Distance (The most important signal)
        dist_component = -distance_to_goal * 100

        # Secondary: Smoothness (Weighted very low so it doesn't stop movement)
        # Only penalize extreme 'jerky' movements
        velocity_penalty = -0.001 * np.linalg.norm(velocity)

        # Tertiary: Efficiency
        time_penalty = -0.01

        # Success
        bonus = 250.0 if distance_to_goal < self.target_threshold else 0.0

        return float(dist_component + velocity_penalty + time_penalty + bonus)
