...
class OT2Env(gym.Env):
    ...
    def _calculate_reward(self, current_pos, distance_to_goal):
        """
        SPARSE + SHAPED PENALTY REWARD (Level 2)
        Binary success signal with quadratic penalty for failures
        Theory: Strong goal signal + nonlinear distance guidance
        """
        if distance_to_goal < self.target_threshold:
            # Sparse success bonus
            reward = 100.0
        else:
            # Quadratic penalty: being far away is much worse
            reward = -(distance_to_goal ** 2) * 100
            # Time penalty
            reward -= 0.01
