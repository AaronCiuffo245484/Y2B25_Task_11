...
class OT2Env(gym.Env):
    ...
    def _calculate_reward(self, current_pos, distance_to_goal):
        gamma = 0.99
        # Linear potential is much more stable for this workspace size
        current_phi = -distance_to_goal
        prev_phi = -self.previous_distance

        # Pure Potential-Based Shaping formula
        shaping = (gamma * current_phi) - prev_phi

        # Scale shaping so the signal is strong (0.1m move = ~5.0 reward)
        shaping *= 100

        # Dense base reward to help the agent find the 'cone' of the potential
        base_reward = -distance_to_goal

        if distance_to_goal < self.target_threshold:
            return 100.0 + shaping
