...
parser.add_argument("--target_threshold", type=float, default=0.015)
parser.add_argument("--reward_type", type=str, default='simple',
                    choices=['simple', 'progressive', 'potential', 'sparse', 'multiobjective'],
                    help='Which reward algorithm to use')
...
# DYNAMIC IMPORT: Load the correct wrapper based on reward_type
print(f"\n{'='*70}")
print(f"LOADING REWARD ALGORITHM: {args.reward_type.upper()}")
print(f"{'='*70}\n")

if args.reward_type == 'simple':
    from filipp_wrapper_simple import OT2Env
elif args.reward_type == 'progressive':
    from filipp_wrapper_progressive import OT2Env
elif args.reward_type == 'potential':
    from filipp_wrapper_potential import OT2Env
elif args.reward_type == 'sparse':
    from filipp_wrapper_sparse import OT2Env
elif args.reward_type == 'multiobjective':
    from filipp_wrapper_multiobjective import OT2Env
else:
...
class OT2Callback(BaseCallback):
    def __init__(self, threshold=0.015, verbose=0):
...
filename = f"{timestamp}_{PERSON_NAME}_{args.reward_type}_lr{lr_str}_b{args.batch_size}_s{args.n_steps}"

print("="*70)
print(f"Training Configuration:")
print(f"  Person: {PERSON_NAME}")
print(f"  REWARD ALGORITHM: {args.reward_type.upper()} ***")
...
print(f"\nTraining complete for {args.reward_type.upper()} reward!")
