...
class OT2Env(gym.Env):
    ...
    def _reward_normalized_progress(self, distance_to_goal, action):
        """
        FIXED: Properly scaled for 1mm precision
        ...
        # Large success bonus with efficiency scaling
    ...
    def _reward_exponential(self, distance_to_goal):
        """
        FIXED: Uses tanh to prevent explosion
        - Safe exponential using hyperbolic tangent
        - Scaled appropriately for 1mm precision
        """
        params = self.reward_params
        ...
        # Use tanh instead of exp to prevent explosion
        # tanh maps to [-1, 1], scale by alpha
    ...
    def _reward_staged(self, distance_to_goal):
        """
        FIXED: Thresholds properly aligned with 1mm goal
        - Progressive milestones leading to 1mm
        - Larger bonuses for achieving milestones
        """
        params = self.reward_params
        reward = 0.0
    ...
    def _reward_dense_shaping(self, distance_to_goal):
        """
        FIXED: Stronger shaping for 1mm precision
        - Increased shaping scale
        - Larger success bonus
        """
        params = self.reward_params
        
        # Potential-based shaping with stronger scale
    ...
    def _reward_energy_efficient(self, distance_to_goal, action):
        """
        FIXED: Reduced action penalties for better learning
        - Smaller penalties on action magnitude
        - Progress still primary signal
        """
        params = self.reward_params
        ...
        # Reduced action penalties (was killing learning)
    ...
    def _get_default_reward_params(self, reward_type):
        """
        FIXED: All parameters properly scaled for 1mm precision
        Success bonuses are dominant to encourage reaching goal
        """
        defaults = {
            'normalized_progress': {
                'progress_scale': 300,      # 3x stronger than before
                'time_penalty': 0.05,        # Reduced penalty
                'efficiency_multiplier': 1.0, # Bigger efficiency reward
                'success_bonus': 200         # Huge success bonus (was 50)
            },
            'exponential': {
                'alpha': 5.0,               # Controls gradient steepness
                'scale': 100,               # Overall scale for tanh function
                'time_penalty': 0.05,       # Small time penalty
                'success_bonus': 250        # Very large success bonus
            },
            'staged': {
                'thresholds': [0.05, 0.02, 0.005, 0.002, 0.001],  # Progressive to 1mm
                'bonuses': [20, 40, 80, 120, 200],  # Increasing bonuses
                'time_penalty': 0.05        # Small time penalty
            },
            'dense_shaping': {
                'shaping_scale': 300,       # 3x stronger shaping (was 100)
                'time_penalty': 0.05,       # Small time penalty
                'success_bonus': 200        # Large success bonus
            },
            'energy_efficient': {
                'progress_scale': 300,      # Strong progress signal
                'action_penalty': 0.02,     # Much smaller (was 0.1)
                'smoothness_penalty': 0.02, # Much smaller (was 0.1)
                'time_penalty': 0.05,       # Small time penalty
                'success_bonus': 200        # Large success bonus
            }
        }
